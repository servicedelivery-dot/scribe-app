...
export async function GET(
  ...
  const rows = await db
    ...
    .where(eq(lmsCourseGroups.groupId, params.id))
...
export async function POST(
  ...
  const existing = await db
    .select()
    .from(lmsCourseGroups)
    .where(and(eq(lmsCourseGroups.groupId, params.id), eq(lmsCourseGroups.courseId, courseId)))

  if (existing.length > 0) return NextResponse.json({ error: 'Already in group' }, { status: 409 })

  const [row] = await db.insert(lmsCourseGroups).values({
    groupId: params.id,
    courseId,
  }).returning()
